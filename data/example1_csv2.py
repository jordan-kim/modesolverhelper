...
def find_n_eff(wg_width:float)->float:
    ...
    mode_solver = ms.ModeSolverSemiVectorial(4, semi_vectorial_method='Ey') 
    a = mode_solver.solve(structure)
    return a["n_effs"][1].real

for i in w_wg_list:
    l = find_n_eff(i)
    total_list.append([i,l])

df = pd.DataFrame(total_list, columns=['w_wg_list','n_eff'])

df.plot.scatter(x='w_wg_list', y='n_eff')
plt.ylim([2.5,3])
plt.show()